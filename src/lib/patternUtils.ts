...
export function generateColorMap(
  ...
  // Always use horizontal logic for color pattern generation
  // Vertical orientation is handled by 3D model rotation, not color pattern logic
  const effectiveOrientation = "horizontal";
  ...
  if (effectiveColorPattern === "fade") {
    ...
    if (isRotated) {
      ...
      const rowOrder = Array.from({ length: adjustedModelHeight }, (_, i) => i);
  ...
  } else if (effectiveColorPattern === "center-fade") {
